...
describe MixpanelAPI::Client do
  before :all do
    @client = MixpanelAPI::Client.new(:api_key => 'test_key', :api_secret => 'test_secret')
    @uri = Regexp.escape(MixpanelAPI::Client::BASE_URI)
  end

  context 'when initializing a new MixpanelAPI::Client' do
    it 'should not raise an exception if a hash is given' do
      MixpanelAPI::Client.new('api_key' => 'test_key', 'api_secret' => 'test_secret').should_not raise_error(ArgumentError)
    end

    it 'should set a parallel option as false by default' do
      MixpanelAPI::Client.new(:api_key => 'test_key', :api_secret => 'test_secret').parallel.should == false
    end

    it 'should be able to set a parallel option when passed' do
      MixpanelAPI::Client.new(:api_key => 'test_key', :api_secret => 'test_secret', :parallel => true).parallel.should == true
    end
  end
  ...
  context 'when making a valid request' do
    ...
    context "with parallel option enabled" do
      before :all do
        @parallel_client = MixpanelAPI::Client.new(:api_key => 'test_key', :api_secret => 'test_secret', :parallel => true)
      end
  ...
  describe '#hash_args' do
    it 'should return a hashed string alpha sorted by key names.' do
      ...
      MixpanelAPI::Client::Utils.generate_signature(args, @client.api_secret).should == MixpanelAPI::Client::Utils.generate_signature(args_alpha_sorted, @client.api_secret)
    end
  end

  describe '#to_hash' do
    it 'should return a ruby hash given json as a string' do
      MixpanelAPI::Client::Utils.to_hash('{"a" : "ey", "b" : "bee"}', :json).should == {'a' => 'ey', 'b' => 'bee'}
    end
  end
